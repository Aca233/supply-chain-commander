...
describe('china 2019 industrial baseline regression guards', () => {
  it('keeps infrastructure throughput ahead of frontier and luxury output', () => {
    ...
    expect(smartphoneDaily).toBeGreaterThan(chipDaily * 3);
  ...
  it('keeps sector cost structure aligned with industry profiles', () => {
    ...
    expect(steelMill!.energyCost).toBeGreaterThan(steelMill!.laborCost);
    expect(refinery!.energyCost).toBeGreaterThan(refinery!.laborCost);
    expect(semiconductorFab!.maintenanceCost).toBeGreaterThan(electronicsFactory!.maintenanceCost * 4);
    expect(pharmaFactory!.maintenanceCost).toBeGreaterThan(pharmaFactory!.energyCost);
  ...
  it('keeps construction capital density centered on heavy industry and coal power', () => {
    ...
    expect(powerPlant!.buildCost).toBeGreaterThan(luxuryWorkshop!.buildCost);
    expect(steelMill!.buildCost).toBeGreaterThan(buildingMaterialsFactory!.buildCost * 4);
